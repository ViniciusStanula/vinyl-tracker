...
_KEY = "a42e4483f1d942f99203b177055c71a4"
_HOST = "www.garimpavinil.com.br"
_BASE = "https://www.garimpavinil.com.br"
_KEY_LOCATION = f"{_BASE}/{_KEY}.txt"
...
def build_urls(items: list[dict], db_estilos: list[str] | None = None) -> list[str]:
    """Build all frontend URLs from crawled items: disco + artist + style pages.

    `db_estilos` is a list of raw lastfm tag strings from the DB
    (each is a comma-separated tag list like "Rock, Classic Rock").
    Required because `estilo` is not present in crawled item dicts.
    """
    urls: list[str] = []
    ...
    for item in items:
        slug = item.get("slug")
        if slug:
            urls.append(f"{_BASE}/disco/{slug}")

        artista = item.get("artista") or ""
        if artista:
            artist_slug = _slugify_artist(artista)
            if artist_slug and artist_slug not in seen_artists:
                seen_artists.add(artist_slug)
                urls.append(f"{_BASE}/artista/{artist_slug}")

    for raw in (db_estilos or []):
        for tag in raw.split(", "):
            ...
            if style_slug and style_slug not in seen_styles:
                ...
                urls.append(f"{_BASE}/estilo/{style_slug}")

    log.info(
        "IndexNow: built %d URLs (%d disco, %d artist, %d style).",
        len(urls),
...
def submit_crawl_results(items: list[dict], db_estilos: list[str] | None = None) -> None:
    """Build and submit all frontend URLs derived from crawled items.

    Pass `db_estilos` (raw lastfm tag strings from the Disco table) to also
    submit style pages — they are not available on crawled item dicts.
    """
    if not items:
        log.info("IndexNow: no items — skipping submission.")
        return
    urls = build_urls(items, db_estilos)
    submit_urls(urls)
